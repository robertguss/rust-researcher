...
pub async fn report_pdf(chrome: &Path, html: &str) -> Result<Vec<u8>, AppError> {
    let directory = tempfile::tempdir()?;
    let input = directory.path().join("report.html");
    ...
    .await
    .map_err(|_| AppError::validation("render_timeout", "PDF renderer exceeded 30 seconds"))??;
    if !result.status.success() {
        return Err(AppError::validation(
            "render_failed",
            format!("PDF renderer exited with {}", result.status),
        ));
    }
    let pdf = tokio::fs::read(output).await?;
    if !pdf.starts_with(b"%PDF-") || pdf.len() > 20 * 1024 * 1024 {
        return Err(AppError::validation(
            "invalid_pdf",
            "renderer output is not a valid bounded PDF",
        ));
    }
    Ok(pdf)
}
...
@media print{body{background:white;font-size:14px;line-height:1.45}main{width:auto;padding:0;box-shadow:none}h1{font-size:2.4rem}.claim{break-inside:avoid}.report-footer{break-inside:avoid;margin-top:24px}}
"#;
